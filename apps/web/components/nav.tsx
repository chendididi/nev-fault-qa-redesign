...
export function Nav() {
  ...
  return (
    <aside className="glass-panel hidden h-[calc(100vh-48px)] w-64 shrink-0 flex-col p-4 md:flex">
        ...
        {links.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex h-12 items-center gap-3 rounded-xl border px-3 text-sm font-semibold ${active ? "border-[color-mix(in_srgb,var(--primary-color)_24%,transparent)] bg-[color-mix(in_srgb,var(--primary-color)_12%,var(--floating-surface))] text-ink" : "border-transparent text-ink hover:border-line hover:bg-[color-mix(in_srgb,var(--text-primary)_7%,transparent)]"}`}
            >
      ...
      <button
        className="secondary-button flex h-11 items-center gap-3 rounded-xl px-3 text-sm font-semibold"
        onClick={() => {
          clearToken();
          router.push("/login");
        }}
